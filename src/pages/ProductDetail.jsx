    ...
    useEffect(() => {
        const fetchProductData = async () => {
            ...
            try {
                ...
                const mappedMovements = Array.isArray(movementsData) && movementsData.length > 0 ? movementsData.map(m => ({
                    date: new Date(m.createdAt || m.date).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }),
                    reference: m.referenceId || m.reference || m.id?.substring(0, 8),
                    type: m.reason || (m.type === 'IN' ? 'Purchase' : 'Sale'),
                    adjustment: m.type === 'OUT' ? -Math.abs(m.quantity) : Math.abs(m.quantity)
                })) : [];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                    ...
                    <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-sm text-slate-500">
                        <span>Showing {movements.length} movement{movements.length !== 1 ? 's' : ''}</span>
                    </div>
                </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-sm font-bold text-slate-800 dark:text-white">{(product.stockQuantity || 0).toLocaleString()}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-sm font-bold text-slate-800 dark:text-white">
                                    ${((product.price || 0) * (product.stockQuantity || 0)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                                </span>
                            </div>
                        </div>
                    </div>
